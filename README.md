...
As a minimum you must have the CCF_DATA EXML file that matches your vehicle and decrypted it to an XML file (handy util for that here: https://github.com/5555l/sdd_exml). This file can be discovered automatically or must be specified using `--xml` argument. It must be given the CCF to decode, by default output will be returned to stdout as a string. If a `VIN` is not specified it tries to grab it from the CCF directly itself.
...
|`-i / --can_id`|string|canID used in the can dump to broadcast CCF, default = `401` (Jag MS CAN)|
|`-u / --can_id_eucd`|string|canID used in the can dump to broadcast CCF_EUCD, default = `402` (Jag MS CAN)|
